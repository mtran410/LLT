...
public class PlayingPage extends Activity {
	...
	private ArrayList<Integer> id = new ArrayList<Integer>();// id number for
																// identification
	private ArrayList<Integer> playerDeath = new ArrayList<Integer>();
	TextView name1, name2, name3, name4, name5, name6, teamName, win, loss;// to
																			// change
																			// the
																			// players
																			// name
	TextView death1, death2, death3, death4, death5, death6;// to change the
															// number of death
	Button health1, health2, health3, health4, health5, health6;// to change the
																// health Bar
	LinearLayout l1, l2, l3, l4, l5, l6;
	TextView player1Image, player2Image, player3Image, player4Image,
			player5Image, player6Image;// to change the image
	ArrayList<Boolean> playerDead = new ArrayList<Boolean>();
	boolean light = false;
	float lightAmount = 300f;	Double subHealth = 0.01;
	int numberPlayer = 0, teamNumber = 2;
	private Integer numWin = 9, numLoss = 4;
// how much to subtract the health by. Player start
							// with 1
	...
	private class LongOperation extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			if (firstTime) {
				...
				temp = "http://www.jonquybao.com/LLT/feedurls/all_players.php?pid=1&health="
						+ playerHealth.get(0) * 100;
		...
		protected void onPostExecute(String result) {
			...
			;
			Log.d("DIPLAY PLAYER COUNT", players.get(0).getIGN() + "");
			...
			setPlayerHealth((double) players.get(0).getHealth() / 100, 0);
			setPlayerHealth((double) players.get(1).getHealth() / 100, 1);
			setPlayerHealth((double) players.get(2).getHealth() / 100, 2);
	...
	public void setPlayerHealth(Double lessHealth, Integer playerNum) { // method
																		// change
																		// the
																		// player
																		// health
		...
		if (playerNum == id.get(0)) {// check which player health to change
			...
			if (playerDead.get(0) == false) {// if player is still alive then
												// change health if not do
												// nothing
				if (lessHealth < 0.0)
					lessHealth = 0.0;// change change bar to under 0
				health1.setScaleX(Float.parseFloat(lessHealth.toString()));// change
																			// health
																			// bar
				health1.setText(healthFormat.format(lessHealth * 100)
						.toString() + "%");// type percentage health on bar
				if (lessHealth <= 0.0) {// if player die then set to true
					playerDead.set(0, true);
					player1Image.setText(" X"); // put an x on player pic
					playerDeath.set(0, playerDeath.get(0) + 1);// add to death
																// count
					death1.setText(playerDeath.get(0).toString());// set number
																	// of death
																	// to screen
					// v.vibrate(300);
		...
		if (playerNum == id.get(1)) {
			if (playerDead.get(1) == false) {
				...
				health2.setText(healthFormat.format(lessHealth * 100)
						.toString() + "%");
				if (lessHealth == 0.0) {
		...
		if (playerNum == id.get(2)) {
			...
			if (playerDead.get(2) == false) {
				...
				health3.setText(healthFormat.format(lessHealth * 100)
						.toString() + "%");
				if (lessHealth == 0.0) {
		...
		if (playerNum == id.get(3)) {
			if (playerDead.get(3) == false) {
				...
				health4.setText(healthFormat.format(lessHealth * 100)
						.toString() + "%");
				if (lessHealth == 0.0) {
		...
		if (playerNum == id.get(4)) {
			if (playerDead.get(4) == false) {
				...
				health5.setText(healthFormat.format(lessHealth * 100)
						.toString() + "%");
				if (lessHealth == 0.0) {
		...
		if (playerNum == id.get(5)) {
			if (playerDead.get(5) == false) {
				...
				health6.setText(healthFormat.format(lessHealth * 100)
						.toString() + "%");
				if (lessHealth == 0.0) {
	...
	// set the player name
	public void setName() {
		name2.setText(players.get(1).getIGN());
		name1.setText(players.get(0).getIGN());
		name3.setText(players.get(2).getIGN());
		name4.setText(players.get(0).getIGN());
		name5.setText(players.get(0).getIGN());
		name6.setText(players.get(0).getIGN());
	}
